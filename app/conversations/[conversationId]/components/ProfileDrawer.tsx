...
const ProfileDrawer = ({ data, isOpen, onClose }: ProfileDrawerProps) => {
  ...
  const statusText = useMemo(() => {
    const connectedMembersCount = data.users.filter((user) =>
      members.includes(user.email!)
    ).length;
    console.log(connectedMembersCount);
    if (data.isGroup) {
      if (connectedMembersCount) {
        return `(${connectedMembersCount}/${data.users.length}) members online`;
      }
      return `All members(${data.users.length}) offline`;
    }
